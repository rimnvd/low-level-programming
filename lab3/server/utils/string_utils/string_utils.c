...
char* concat_strings(const char* first_str, const char* second_str) {
    char* result_string = get_pointer(strlen(first_str) + strlen(second_str) + 1);
    strcpy(result_string, first_str);
    strcat(result_string, second_str);
...
size_t split(char* string, char c, char*** array) {
    ...
    *array = (char**) get_pointer(sizeof(char*)* cnt);
    ...
    int i = 0;
    char* start = string;
    for (position = string; *position != '\0'; position++) {
        if (*position == c) {
            *position = '\0';
            (*array)[i++] = start;
            start = position + 1;
