...
size_t get_id_array_size(uint64_t current_id, uint64_t pattern_size) {
    ...
    size_t size = max((whole + fractional) * tuple_size / OFFSET_VALUE_SIZE,
                      MIN_ID_ARRAY_SIZE * tuple_size / OFFSET_VALUE_SIZE);
    return size;
}

static enum read_file_status read_tree_subheader(struct tree_subheader* subheader, FILE* f) {
    return read_from_file(subheader, f, sizeof(struct tree_subheader));
}
...
enum read_file_status read_tuple(uint64_t pattern_size, struct tuple **tuple, FILE *file) {
    *tuple = (struct tuple *) malloc(sizeof(struct tuple));
    enum read_file_status status = read_from_file(*tuple, file, sizeof(union tuple_header));
    (*tuple)->data = (uint64_t*) malloc(get_tuple_size(pattern_size));
    status |= read_from_file((*tuple)->data, file, get_tuple_size(pattern_size));
    return status;
}
